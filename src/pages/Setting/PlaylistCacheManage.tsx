...
import { formatRelativeTime, formatSize } from '../../util';
...
const PlaylistCacheManagePage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const result = await invoke<CachedMusicInfo[]>('get_cached_music_for_playlist', {
  ...
  const handleClear = () => {
    ...
    modalApi.confirm({
      ...
      onOk: async () => {
        try {
          if (currentMusic && selectedRowKeys.includes(currentMusic.song_id) || selectedRowKeys.length === 0) handleClose();
  ...
  const columns: TableProps<CachedMusicInfo>['columns'] = [
    {
      ...
      render: (text, record) => (
        <Flex>
          <img src={record.cover_url || '/icon.png'} alt={text} style={{ width: 40, height: 40, marginRight: 10, borderRadius: 4 }} />
          <Flex vertical justify='center'>
            <Text style={{ maxWidth: 120 }} ellipsis={{ tooltip: text }}>{text}</Text>
            <Text type="secondary" style={{ maxWidth: 120 }} ellipsis={{ tooltip: record.artist }}>{record.artist}</Text>
          </Flex>
        </Flex>
      ),
    },
    {
      ...
      key: 'last_played',
      sorter: (a, b) => new Date(a.last_played_at || 0).getTime() - new Date(b.last_played_at || 0).getTime(),
      // 使用新的时间格式化函数
  ...
  return (
    <Flex vertical style={{ height: '100%', background: '#fff' }}>
      ...
      <Flex
        ...
        style={{ padding: '10px 15px', borderBottom: '1px solid #f0f0f0', flexShrink: 0 }}
      >
        <Space>
          <ArrowLeftOutlined style={{ marginRight: '16px', color: primaryThemeColor, fontSize: '16px' }}
            onClick={() => navigate(-1)} />
          <Title level={5} style={{ margin: 0 }} ellipsis={{ tooltip: playlistName }}>{playlistName}</Title>
        </Space>
      ...
      <Flex justify="space-between" align="center" style={{ padding: '10px 15px', flexShrink: 0 }}>
        <Search
          placeholder="搜索歌名或歌手"
