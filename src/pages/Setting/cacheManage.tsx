...
import { Button, Card, Checkbox, Divider, Flex, Form, List, Slider, Space, Spin, Statistic, Typography } from 'antd';
...
const CacheManagePage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const [
        size, nonPlaylist, oldCache, playlists,
        threshold, exclude,
        cleanOld, // <-- 新增
      ] = await Promise.all([
        ...
        invoke<string | null>('get_app_setting', { key: 'auto_clean_threshold_gb' }),
        invoke<string | null>('get_app_setting', { key: 'auto_clean_exclude_in_playlist' }),
        invoke<string | null>('get_app_setting', { key: 'auto_clean_old_files_enabled' }), // <-- 新增
      ]);
      setTotalSize(size);
  ...
  const handleSaveAutoCleanSettings = async (values: any) => {
    try {
      ...
      await invoke('save_app_setting', { key: 'auto_clean_threshold_gb', value: String(threshold) });
      await invoke('save_app_setting', { key: 'auto_clean_exclude_in_playlist', value: String(exclude) });
      await invoke('save_app_setting', { key: 'auto_clean_old_files_enabled', value: String(cleanOld) }); // <-- 新增
  ...
  return (
    <Spin spinning={loading}>
      <Flex vertical gap="16px">
        ...
        <Card title="智能清理">
          <Form form={form} title='自动清理' layout="vertical" onFinish={(values) => handleSaveAutoCleanSettings(values).then(() => fetchData())}>
            <Form.Item
              label="缓存上限"
            ...
            >
              <Space align="center">
                ...
                <Text>
                  {form.getFieldValue('threshold') === 0 ? '已禁用' : `超过${form.getFieldValue('threshold')} GB自动清理`}
                </Text>
              </Space>
            </Form.Item>
            ...
            <Form.Item>
              <Button type="primary" htmlType="submit">保存自动清理设置</Button>
            </Form.Item>
          </Form>
