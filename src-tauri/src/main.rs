...
#[tauri::command]
async fn get_play_url(song_id: String, title: String, artist: String, keyword: String) -> Result<String, String> {
    ...
    let res = client.post("https://www.gequhai.net/api/play-url")
        .form(&params)
        .send()
        .await
        .map_err(|e| format!("请求播放API失败: {}", e))?;
    
    let play_response: PlayUrlResponse = res.json().await
...
async fn download_song(app: AppHandle, song_id: String, title: String, artist: String, keyword: String) -> Result<(), String> {
    ...
    // --- 核心改动开始 ---
    let resp = reqwest::get(&final_download_url).await.map_err(|e| e.to_string())?;
...
fn main() {
    tauri::Builder::default()
        ...
        // 在这里添加 get_play_url
        .invoke_handler(tauri::generate_handler![search_music, download_song, get_play_url])
